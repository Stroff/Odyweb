...
<?php include "navbar.php"; ?>
	...
	if (isset($_POST['email'])) {
		...
		if ( $_POST ["password2"]=='' && $_POST ["password1"]=='') {
			...
			$connexion = mysql_connect($host_site, $user_site , $pass_site);
			mysql_select_db($site_database ,$connexion);
			$resReqSite = mysql_query ( "UPDATE accounts SET extension = '".$extension."', email = '".$email_edit_compte."' WHERE id='" . $id_compte_edit . "' LIMIT 1" ) or die ( mysql_error () );
			...
			if ($_POST ["password2"]==$_POST ["password1"] && strlen($_POST ["password2"])>5 ) {
				...
				$connexion = mysql_connect($host_site, $user_site , $pass_site);
				mysql_select_db($site_database ,$connexion);
				$resReqWow = mysql_query ( "UPDATE accounts SET password='" . $hash_new_nouveau_password . "', password_old='', extension = '".$extension."', email = '".$email_edit_compte."' WHERE id='" . $id_compte_edit . "' LIMIT 1" ) or die ( mysql_error () );
