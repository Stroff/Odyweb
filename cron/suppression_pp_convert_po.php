...
$sql = mysql_query("SELECT username, pp,points FROM site.accounts2");
// pour tous les comptes
while($un_compte = mysql_fetch_array($sql)) {
	$new_points = $un_compte['points']+round($un_compte['pp']/250,2);
	$sql_maj = "UPDATE site.accounts2 SET pp = 0, points = $new_points WHERE username='".$un_compte['username']."'";
	echo $sql_maj;
	mysql_query($sql_maj);
}
