...
switch ($_POST['type']) {
	...
	$prix_points = 4+mysql_num_rows($renames)*2;
	...
	if ($guid_perso =='') {
		...
		if (mysql_num_rows($persos) == 1) {
				...
				if ($compte_points >=$prix_points) {
					...
					mysql_select_db($site_database ,$connexion);
					mysql_query("SET NAMES 'utf8'");
					$resReqSite = mysql_query ( "INSERT INTO logs_achat_boutique SET date = NOW(), ip='".get_ip()."', account_id = '".$id_compte."', objet_id='Changement de race', perso_id ='" . $guid_perso . "',perso_nom='".$perso['name']."'" ) or die ( mysql_error () );
					...
					if ($resReqSite && $resReqWow&& $resReqSite2) {
						echo "<div id='success_page'>";
						 echo "<p>Vous avez bien fait la demande, lors de votre prochaine connexion vous pourrez changer la race de votre personnage.</p>";
						 echo "</div>";
