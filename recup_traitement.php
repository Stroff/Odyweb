...
if($pseudo==''||$id_perso_cible==""||$serveur==''||$classe==''||$level==''||$level>80||$level<60) {
	...
	if($cout>$compte_pp) {
		$message .= "Vous n'avez pas assez de Points Parrainage. Pour une récupération individuel c'est 30pp et pour une récupération de guilde c'est 10pp.";
	} else {
		if($lvl_metier_secondaire1<=450&&$lvl_metier_secondaire1>=0&&$lvl_metier_secondaire2<=450&&$lvl_metier_secondaire2>=0&&$lvl_metier_secondaire3<=450&&$lvl_metier_secondaire3>=0&&$lvl_metier1<=450&&$lvl_metier1>=0&&$lvl_metier2<=450&&$lvl_metier2>=0&&in_array($metier1,$metiers)&&in_array($metier2,$metiers)&&in_array($metier_secondaire1,$metiers)&&in_array($metier_secondaire2,$metiers)&&in_array($metier_secondaire3,$metiers)&&$lvl_mount<=300&&$lvl_mount>=0) {
			$connexion = mysql_connect($host_wow, $user_wow , $pass_wow);
			mysql_select_db($wow_characters ,$connexion);
			mysql_query("SET NAMES 'utf8'");
			$persos = mysql_query("SELECT name FROM characters WHERE guid = '".$id_perso_cible."' AND account='".$compte_id."'");
